...
Action:CmdBonus(client, args)
{
	...
	if (StrEqual(sCmdType, "full"))
	{
		...
		{
			PrintToChat(client, "%s\x01回合\x04#1\x01 分数: \x05%d\x01/\x05%d\x01 <\x03%.1f%%\x01> [%s]", PLUGIN_TAG, RoundToFloor(fSurvivorBonus[0]), RoundToFloor(fMapBonus + fMaxPillsBonus), CalculateBonusPercent(fSurvivorBonus[0]), sSurvivorState[0]);
		}
	...
	else
	{
		if (InSecondHalfOfRound())
		{
			PrintToChat(client, "%s\x01回合\x04#1\x01 分数: \x05%d\x01 <\x03%.1f%%\x01>", PLUGIN_TAG, RoundToFloor(fSurvivorBonus[0]), CalculateBonusPercent(fSurvivorBonus[0]));
		}
