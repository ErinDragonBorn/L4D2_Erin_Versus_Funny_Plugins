...
void ProcessPlayerLerp(int client, bool load = false, bool team = false)
{
	...
	if ((FloatCompare(newLerpTime, cVarMinLerp.FloatValue) == -1)  || (FloatCompare(newLerpTime, cVarMaxLerp.FloatValue) == 1)) {
		...
		if (!load) {
			...
			if (cVarBadLerpAction.IntValue == 1) {
				CPrintToChatAllEx(client, "{default}<{olive}Lerp{default}> {teamcolor}%N {default}因为 {teamcolor}%.01f lerp被移至旁观者 ", client, newLerpTime * 1000);
				...
				CPrintToChatAllEx(client, "{default}<{olive}Lerp{default}> {teamcolor}%N {default}因为 {teamcolor}%.01f lerp被踢出", client, newLerpTime * 1000);
				KickClient(client, "Lerp限制:(最小值: %.01f, 最大值: %.01f)", cVarMinLerp.FloatValue * 1000, cVarMaxLerp.FloatValue * 1000);
			}
		}
